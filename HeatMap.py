...
# Define the function to plot and save heat maps
def plot_heatmap_for_venue(data, venue=None):
    if venue:
        # Filter data for the specified venue
        filtered_data = data[data['Venue'] == venue]
        title = f'Volume Composition by Asset Class for {venue} (2022 vs 2023) in Percentage'
        filename = f'heatmap_{venue}.png'
    else:
        # Use all data for the total CME Group Exchanges heat map
        filtered_data = data
        title = 'Total Volume Composition by Asset Class for CME Group Exchanges (2022 vs 2023) in Percentage'
        filename = 'heatmap_total.png'

    # Pivot the data
    pivot_data = filtered_data.pivot_table(values='Volume', index='Asset Group', columns='Year', aggfunc='sum')

    # Calculate percentages
    pivot_data_percentage = pivot_data.div(pivot_data.sum(axis=0), axis=1) * 100

    # Plotting the heat map using the custom colormap
...
# Plot heat maps for each individual venue and the total
venues = ['CME', 'CBOT', 'NYMEX', 'COMEX', None]  # Include None for the total CME Group Exchanges
for venue in venues:
    plot_heatmap_for_venue(data, venue=venue)
